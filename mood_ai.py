...
# ----------------------------
# Count Mood Frequencies & Percentages
# ----------------------------
...
dominant_mood = max(combined_percentages.items(), key=lambda x: x[1])[0]
...
# ----------------------------
# AI Suggestions using KMeans Clustering
# ----------------------------
df = pd.DataFrame(daily_logs)
relevant_features = ["sleep", "water", "exercise", "pain", "energy", "mood"]

if not all(col in df.columns for col in relevant_features):
    print("Some required fields are missing in the data.")
    exit()
...
for col in df_features.columns:
    ...
    if abs(good_val - bad_val) >= 0.5:
        ...
        if col == "sleep":
            suggestions.append(f"• You tend to feel better when your sleep quality is {to_val.lower()} instead of {from_val.lower()}.")
        elif col == "water":
            suggestions.append(f"• Drinking water regularly is linked to a more positive mood.")
        elif col == "exercise":
            suggestions.append(f"• Engaging in exercise appears to boost your mood.")
        elif col == "pain":
            suggestions.append(f"• Less pain is associated with better emotional wellbeing.")
        elif col == "energy":
            suggestions.append(f"• Higher energy levels contribute to a more stable or happier mood.")
        else:
            suggestions.append(f"• Changing '{col}' from '{from_val}' to '{to_val}' might improve mood.")

if not suggestions:
    suggestions.append("• You're maintaining consistent mood-supporting habits. Keep it up! 🎉")

# ----------------------------
# Enhanced Summary with Dominant Mood
# ----------------------------
with open("mood_summary_and_ai.txt", "w", encoding="utf-8") as f:
    ...
    f.write("\n🔮 Dominant Mood (Combined)\n")
    f.write("============================\n")
    f.write(f"Most frequent mood this week: *{dominant_mood}* ({combined_percentages[dominant_mood]:.1f}%)\n")

    f.write("\n💡 AI-Based Mood Improvement Suggestions\n")
    f.write("========================================\n")
